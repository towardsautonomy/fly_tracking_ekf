...
class EKF(object):
  ...
  def process_noise_covariance():
    """ Covariance matrix Q for process noise.
    Output:
  ...
  def observation_noise_covariance():
    """ Covariance matrix R for observation noise.
    Output:
  ...
  def observation(state):
    """ Function h, from state to noise-less observation. 
    Input:
...
if __name__ == "__main__":
    ...
    np.random.seed(402)
    ekf = EKF()
    states, observations = ekf.simulation()
    # plotting
    fig = plt.figure()
    ...
    observations = np.load('./data/measurement.npy')
    filtered_state_mean, filtered_state_sigma, predicted_observation_mean, predicted_observation_sigma = \
        ekf.process(observations)
        
    # plotting
    true_states = np.load('./data/true_state.npy')
    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
